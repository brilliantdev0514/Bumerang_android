...
public class DetailkampActivity extends BaseActivity implements OnMapReadyCallback {


 String userInfo;
    @BindView(R.id.btn_rent_kamp) Button editandmessage;
    @BindView(R.id.detail_title) TextView detail_title;
    @BindView(R.id.detail_price_camp)  TextView detail_price;
    ...
    @BindView(R.id.txv_address_kamp)TextView txv_address_kamp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detailkamp);

        ButterKnife.bind(this);

        detail_title.setText(Constants.itemModel.title);
        txv_address_kamp.setText(Constants.itemModel.address);
                    ...
                    if (editandmessage.getText().toString().equals("Düzenle")) {
                        ...
                        des.setValue(detail_des.getText().toString()).addOnSuccessListener(new OnSuccessListener<Void>() {
                            ...
                            public void onSuccess(Void aVoid) {
                                Toast.makeText(DetailkampActivity.this, "Başarıyı kaydedin!", Toast.LENGTH_SHORT).show();
                                detail_des.setFocusable(false);
                                detail_des.setFocusableInTouchMode(false);
                                detail_des.setClickable(false);
                                editandmessage.setText("Düzenle");
            ...
            editandmessage.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (Constants.itemModel.rental_state != null && Constants.itemModel.rental_state.equals("Yes")) {
                        ...
                        if (EasyPreference.with(DetailkampActivity.this).getInt("easyRegistStatus", 0) == 1) {
                            Intent intent = new Intent(DetailkampActivity.this, ChattingActivity.class);
        ...
        deletePro.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (Constants.itemModel.owner_id != FirebaseAuth.getInstance().getCurrentUser().getUid()) {
                    ...
                    new AlertDialog.Builder(DetailkampActivity.this)
                            ...
                            .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    deleting.child(String.valueOf(Constants.itemModel.id)).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
                                        public void onSuccess(Void aVoid) {
                                            Intent intent = new Intent(DetailkampActivity.this, HomeActivity.class);
                                            startActivity(intent);
                                            finish();
                                        }
                                    });
...
//                startActivity(intent);
                if(userInfo.length() == 0) {
                    Intent intent = new Intent(DetailkampActivity.this, UserProfileActivity.class);
                    intent.putExtra("owner_id", Constants.itemModel.owner_id);
                    startActivity(intent);
                }else {
                    Intent intent =  new Intent(DetailkampActivity.this, UserProfileBusinessActivity.class);
                    intent.putExtra("owner_id", Constants.itemModel.owner_id);
                    startActivity( intent);
                }
            }
...
//        String owner_id = EasyPreference.with(this).getString("easyUserId", "");
//        if(owner_id.equals(Constants.itemModel.owner_id)){
//            findViewById(R.id.btn_rent_home).setVisibility(View.GONE);
